//Custom Pinout for Arduino Nano 33 IoT for boards.h
//To get to the file, go to Arduino package contents>>Java>>libraries>>Firmata
//Update this code into that file and it should compile
...
// Arduino Nano 33 IoT (Custom Pinout)
#elif defined(ARDUINO_SAMD_NANO_33_IOT)
#define TOTAL_ANALOG_PINS       8
#define TOTAL_PINS              30 // 14 digital + 8 pwm + 8 analog
#define IS_PIN_DIGITAL(p)       ((p) >= 0 && (p) <=29)
#define IS_PIN_ANALOG(p)        ((p) >= 14 && (p) < 14 + TOTAL_ANALOG_PINS)
#define IS_PIN_PWM(p)           digitalPinHasPWM(p)
#define IS_PIN_SERVO(p)         (IS_PIN_DIGITAL(p) && (p) < MAX_SERVOS)
#define IS_PIN_I2C(p)           ((p) == 8 || (p) == 9) // SDA = 8, SCL = 9
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK)
